...
class DevController extends Controller
{
    ...
    public function config_frame_update(Request $req){
        if($req->has('atrribute_id')){
            $attr_id = Attribute::find($req->atrribute_id);
            if($attr_id){
                $frame = Item::where('key_item', 'config_frame_attribute')->first();
                if(!$frame){
                    $frame = new Item;
                    $frame->key_item = 'config_frame_attribute';
                }
                $frame->value = $req->atrribute_id;
                $frame->save();
            }
        }
