(function( $ ){
	    $.fn.stag = function(name,list) {
	       if(!$(this).next().hasClass('stag-box')){
	       		...
	       		$.each(l_item, function(i,v){
	       			if($.trim(v)){
	       				str = '<div class="stag-item">'+'<div class="stag-name">'+'<span>'+v+'</span>'+'</div>'+'<div class="stag-close">'+'<span >×</span>'+'</div>'+'</div>';
	       				str_l_item +=str;
	       			}
	       		});	
		     	l = '';	
	    ...
	    $.fn.stagSuggestList =  function(list){
	    	l = '';
	    	$.each(list,function(i,v){
		     		l += '<div class="stag-autocomplate">'+v+'</div>';
		    });
	    ...
	    // Xóa item 
	   	$.fn.removeStagItem = function(container,item){
	    	$(item).remove();
			$(container).refreshStag();
	    }
	    $(document).on('keydown','.stag-input',function(e){
   			  ...
   			  if (keyCode === 40) { 
			    //down
			    suggest = $(this).getStagContainer().find('.stag-suggest');
			  ...
			  if(keyCode === 13) { 
			    ...
			    if(active.length){
			    	if( $.trim($(active).text() )){
				  		$(this).parent().parent().parent().addStagItem($(active).text());
				  		$(this).val('');
				  	}
			    }else{
			    	if($.trim(text)){
				  		$(this).parent().parent().parent().addStagItem(text);
				  		$(this).val('');
				  	}
			    }
   			  }
			  if (keyCode === 8) { 
				    ...
				    if($(del).hasClass('stag-item')){
				    	$(del).removeStagItem($(del).getStagContainer(),del);
				    }
				}
			  }
			  if (keyCode === 9) { 
   		...
   		$(document).on('click','.stag-autocomplate',function(e){
   			...
   			$(this).parent().parent().parent().parent().addStagItem(val);
   		});
   		$(document).on('click','.stag-close',function(e){
   			$(this).removeStagItem($(this).getStagContainer(),$(this).parent() );
   		});
