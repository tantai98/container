...
<?php
   $comments = App\CommentProduct::select('comment_products.*','accounts.username','products.name','products.slug')->leftJoin('accounts', 'comment_products.account_id', '=', 'accounts.id')->where('comment_products.status',0)->leftJoin('products', 'products.id', '=', 'comment_products.product_id')->orderby('created_at','asc')->paginate(10); 
?>
      ...
      <div class="box">
        ...
        <div class="box-body">
          ...
          <div class="dropdown-menu pull-right">
            <a  class="dropdown-item" href="{{route('comments.product.list')}}">Chưa duyệt</a>
             <a  class="dropdown-item" href="{{route('comments.product.list.done')}}">Đã duyệt</a>
          ...
          <table class="table m-b-none" ui-jp="footable" data-filter="#filter" data-page-size="10">
            ...
            <tbody>
              ...
              <tr>
                  ...
                  <td><a style="" href="{{route('frontend.san-pham.slug',['id'=>$value->product_id,'slug'=>$value->slug])}}" target="_blank">{{$value->name}}</a></td>
                  <td class="action-post" >
                    <div style="min-width:140px">
                      ...
                      <a href="{{route('product.edit.product',['id'=>$value->product_id])}}" target="_blank">
                      SP
                      </a>
                    </div>
                  </td>
...
function xacnhanxoa(msg){
 ...
 $(document).on('click', '.duyet', function(event){
    ...
    if(click_duyet==0){
        ...
        $.ajax({
          type: 'post',
          url:  "{{route('comments.product.accept')}}",
          data: { 'id' : id},
     ...
     if(xacnhanxoa('Bạn có muốn xóa comment này không?')===false){
      ...
      if(click_del==0){
        click_del = 1;
         $.ajax({
             type: 'post',
             url:  "{{route('comments.product.delete')}}",
             data: {'id': id},
             dataType:'json',
             success: function(data){
                click_del = 0;  
