...
@endsection
@section('content')
<form ui-jp="parsley" action="{{route('save.cate.products.edit')}}" method="post" enctype="multipart/form-data">
					...
					<div class="row">
							<div class="col-sm-6">
										...
										<input type="hidden" name="id" value="{{$catId->id}}">
							              ...
							              <input type="text" class="form-control" value="{{$catId->name}}" name="cate_name" required >                        
							            </div>
							           
							            <div class="form-group">
							              <label>Mô tả về danh mục nếu có</label>
							              <textarea name="cate_des" class="form-control" rows="5">{{$catId->description}}</textarea>                       
							            </div>
							            ...
							            <div class="form-group">
									        ...
									        <select id="single" class="form-control select2" name="cate_parent">
									            ...
									            <option @if($catId->id == $v0->id) disabled @endif    @if($catId->parent_id == $v0->id) selected @endif value="{{$v0->id}}">{{$v0->name}}</option>
									            @if($v0->subcategory)
									                 @foreach($v0->subcategory as $v1)
                                                        <option @if($catId->id == $v0->id || $catId->id == $v1->id) disabled @endif  @if($catId->parent_id == $v1->id) selected @endif value="{{$v1->id}}">{{$space}}{{$v1->name}}</option>
                                                         @if($v1->subcategory)
											                 @foreach($v1->subcategory as $v2)
		                                                        <option 
		                                                        @if($catId->id == $v0->id || $catId->id == $v1->id || $catId->id == $v2->id) disabled @endif 
		                                                        @if($catId->parent_id == $v2->id) selected @endif
		                                                         value="{{$v2->id}}">{{$space.$space}}{{$v2->name}}</option>
		                                                        @if($v2->subcategory)
													                 @foreach($v2->subcategory as $v3)
				                                                        <option @if($catId->id == $v0->id || $catId->id == $v1->id || $catId->id == $v2->id || $catId->id == $v3->id) disabled @endif 
				                                                         @if($catId->parent_id == $v3->id) selected @endif  value="{{$v3->id}}">{{$space.$space.$space}}{{$v3->name}}</option>
				                                                        @if($v3->subcategory)
															                 @foreach($v3->subcategory as $v4)
						                                                        <option disabled @if($catId->parent_id == $v4->id) selected @endif  value="{{$v4->id}}">{{$space.$space.$space.$space}}{{$v4->name}}</option>
						                                                     @endforeach
						                                                 @endif
				                                                     @endforeach
				                                                 @endif
		         				...
		         				<div class="col-sm-6">
		         					 <div class="form-group">
							              <label style="margin-bottom:5px;">Ảnh mô tả danh mục nếu có</label>
							               ...
							               <img  id="img_preview" style="max-height:100px;"  @if(strlen($catId->img)) src="{{url($catId->img)}}" @endif style="width:100%;height:auto" >
