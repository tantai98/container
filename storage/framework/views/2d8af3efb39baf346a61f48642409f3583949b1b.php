...
<!-- ############ PAGE START-->
    ...
    .dd-1{
      display: none;
            ...
            ?>
            <?php foreach($list_transpost as $v0): ?>
            ...
            <div  class="dd">
              <ol class="dd-list dd-list-handle root_ol">
                   ...
                   <li class="dd-item" data-id="<?php echo e($v0->id); ?>">
                    <div class="dd-content box">
                      ...
                      <div>
                          <a id="d-block"  data-id="<?php echo $v0->id; ?>" style="margin-right:8px;font-size:10pt;font-weight:Roboto Bold;cursor:pointer;">+</a><div class="menu_name"><?php echo e($v0->name); ?></div>
                          <div class="menu_edit d-abcdf_<?php echo $v0->id; ?>">
                            
                              <div id="d-province-ajax_<?php echo $v0->id; ?>" class="d-price d-abcd_<?php echo $v0->id; ?> <?php if($v0->status == 0): ?> dd-1 <?php endif; ?>" style="float:left;padding-right:10px; "><?php echo number_format((int)$v0->price,0,'','.'); ?>đ
                              </div>
                           
                            <div style="float:right";>
                            <select class="d-selet" id="single" data-id="<?php echo $v0->id; ?>" name="label" tabindex="-5" aria-hidden="true">
                              <option <?php if($v0->status == 0): ?> selected <?php endif; ?> value="0">Giá riêng</option>
                              <option <?php if($v0->status == 1): ?> selected <?php endif; ?> value="1">Giá chung</option>
                            </select>
                            </div>
                            
                              <a id="d-dat-gia-province" class="d-abc_<?php echo $v0->id; ?> <?php if($v0->status == 0): ?> dd-1 <?php endif; ?>" data-id="<?php echo $v0->id; ?>" href="">
                                Đặt Giá
                              </a>
              ...
              <div id="d-config_<?php echo $v0->id; ?>">
              <?php if($v0->status == 0): ?>
               <?php foreach($lis_district as $v1): ?>
                  <ol class="dd-list dd-list-handle child_ol province_<?php echo $v0->id; ?> " style="display:none;margin-left:40px;" >
                      ...
                      <li class="dd-item">
                        <div class="dd-content box">
                          <div>
                              <div class="menu_name"><?php echo $v1->name; ?></div>
                              <div class="menu_edit">
                              <div id="d-district-ajax_<?php echo $v1->id; ?>" class="price_<?php echo $v0->id; ?>" style="float:left;padding-right:100px;"><?php echo number_format((int)$v0->price,0,'','.'); ?>đ</div>
                              </div>
                          </div>
                        </div>
                      </li>
      ...
      id = $(this).data('id');
       if(value == 0){
          $.ajax({
            ...
            type:"post",
            url:"<?php echo e(route('ajax.giarieng')); ?>",
         ...
         $.ajax({
            ...
            type:"post",
            url:"<?php echo e(route('ajax.giachung')); ?>",
            data:{'id':id,'value':value},
            success:function(data){
  ...
  //province
    $(document).on('click','#d-dat-gia-province',function(e){
      ...
      $('#m-a-tab').modal('show');
        ...
        type:"post",
        url:"<?php echo e(route('ajax.datgia')); ?>",
        data:{'id':id},
        success:function(data){
    ...
    $(document).on('click','#d-dat-gia-district',function(e){
      ...
      $('#m-a-tab2').modal('show');
        ...
        type:"post",
        url:"<?php echo e(route('ajax.datgia.district')); ?>",
        data:{'id':id},
        success:function(data){
    ...
    $(document).on('click','#add_price_province',function(e){
      ...
      price = $('#d-price-province').val();
        ...
        type:"post",
        url:"<?php echo e(route('ajax.price.province')); ?>",
     ...
     $(document).on('click','#add_price_district',function(e){
      ...
      price = $('#d-price-district').val();
        ...
        type:"post",
        url:"<?php echo e(route('ajax.price.district')); ?>",
        data:{'id':id,'price':price},
        success:function(data){
          if(data.status == true){
   ...
   // $('#single').select2();
    ...
    //   $.ajax({
    //            type: 'post',
    //            url:  "<?php echo e(route('menu.orderby')); ?>",
    //            data: {'arr': arr},
    //            dataType:'json',
