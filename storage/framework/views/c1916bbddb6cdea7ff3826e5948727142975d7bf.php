...
<?php
    ...
    if (isset($id_cate)) {
      ...
      if (sizeof($cate)) {
          $cate_post = App\Post_category::where('category_id',$id_cate)->get();
          $in = array();
          foreach($cate_post as $item){
            array_push($in,$item->post_id);
          }
          $tag = App\Tag_post::wherein('tag_post.post_id',$in)->groupby('tag_post.tag_id')->leftjoin('tags','tag_post.tag_id','=','tags.id')->select('tags.*')->get();
      }else{
        $tag = App\Tag::get();
      }
    }else{
...
<div class="navbar">
    <div style="float:left;" class="title_form">
        <?php if(isset($id_cate) && sizeof($cate)): ?> 
      ...
      <div class="box">
        ...
        <div class="box-body">
          ...
          <div class="dropdown-menu pull-right">
             <a  class="dropdown-item" href="<?php echo e(route('tags.list')); ?>">Tất cả</a>
            <?php foreach($list_categories as $v0): ?>
              <a  class="dropdown-item" href="<?php echo e(route('tags.detail.cate',['id'=>$v0->id])); ?>"><?php echo e($v0->name); ?></a>
              <?php if($v0->subcategory): ?>
                   <?php foreach($v0->subcategory as $v1): ?>
                        <a  class="dropdown-item"  href="<?php echo e(route('tags.detail.cate',['id'=>$v1->id])); ?>"><?php echo e($space); ?><?php echo e($v1->name); ?></a>
                        <?php if($v1->subcategory): ?>
                          <?php foreach($v1->subcategory as $v2): ?>
                            <a  class="dropdown-item"  href="<?php echo e(route('tags.detail.cate',['id'=>$v2->id])); ?>"><?php echo e($space.$space); ?><?php echo e($v2->name); ?></a>
                                <?php if($v2->subcategory): ?>
                                <?php foreach($v2->subcategory as $v3): ?>
                                  <a  class="dropdown-item" href="<?php echo e(route('tags.detail.cate',['id'=>$v3->id])); ?>"><?php echo e($space.$space.$space); ?><?php echo e($v3->name); ?></a>
                                  <?php if($v3->subcategory): ?>
                                     <?php foreach($v3->subcategory as $v4): ?>
                                      <a  class="dropdown-item" href="<?php echo e(route('tags.detail.cate',['id'=>$v4->id])); ?>"><?php echo e($space.$space.$space.$space); ?><?php echo e($v4->name); ?></a>
                                     <?php endforeach; ?>
                                 <?php endif; ?>
                                 <?php endforeach; ?>
                             <?php endif; ?>
          ...
          <button class="number-post"><?php echo e(sizeof($tag)); ?> tag</button>
              ...
              <tr>
                 ...
                 <!--  <td>
                  ...
                  <td class="action-post">
                    ...
                    <a class="edit" href="#" type="button" data-id="<?php echo e($value->id); ?>">
                    Sửa
                    </a>
                    <a href="#" type="submit" id="xoa-tag" data-id="<?php echo e($value->id); ?>">
                    Xóa
                    </a>
                  </td>
...
function xacnhanxoa(msg){
 ...
 $(document).on('click', '.edit', function(event){
    ...
    $.ajax({
      type: 'get',
      url:  '<?php echo e(route('tags.edit', 'id')); ?>',
      data: { 'id' : id},
 ...
 $(document).on('click','#xoa-tag', function(event){
           ...
           if(xacnhanxoa('Bạn có chắc xóa tag hiện tại hay không?')===false){
            ...
            id = $(this).attr('data-id');
             $.ajax({
                 type: 'post',
                 url:  '<?php echo e(route('tags.del')); ?>',
                 data: {'id': id, 'id_cate' : <?php echo e(isset($id_cate) ? $id_cate : 0); ?> },
                 dataType:'json',
