...
<form ui-jp="parsley" method="post" action="<?php echo e(route('transpost.submit')); ?>" >
...
<div class="navbar">
   ...
   <div class="app-body" id="view">
    ...
    <div class="padding">
         <div class="row">
          <div class="col-sm-6" >
            <div class="box" style="min-height:140px  !important;" >
                  <div class="box-header">
                    <h2 style="font-family:'Roboto Black';">Đặt giá phí vận chuyển</h2>
                  </div>
                  <div class="box-body" style="padding-top:10px;padding-right:20px;">
                        <div class="row">
                            <input type="hidden" value="<?php echo e(csrf_token()); ?>" name="_token">
                             
                                    
                                    <div class="form-group d-1" >
                                          <label class="col-sm-3 form-control-label ">Tỉnh Thành</label>
                                          <div class="col-sm-9">
                                              <select class="form-control d-list-province" name="province">
                                                  <option value="0">Chọn Tỉnh Thành</option>
                                                  <?php foreach($province as $item): ?>
                                                    <option value="<?php echo $item->id; ?>" data-id = "<?php echo $item->id; ?>" ><?php echo $item->name; ?></option>
                                                  <?php endforeach; ?>
                                              </select> 
                                          </div>                       
                                    </div>
                                    <div class="form-group d-a" style="display:none" >
                                           <label class="col-sm-3 form-control-label ">Quận , Huyện</label>
                                              <div class="col-sm-9 d-province">
                                                  
                                              </div> 
                                    </div>
                                    <div class="form-group d-ready" style="display:none">
                                           <label class="col-sm-3 form-control-label ">Phí vận chuyển</label>
                                           <div class="col-sm-9 d-monny">
                                              <input readonly="true" type="text" class="form-control" placeholder="Phí vận chuyển" name="price" autocomplete="off">
...
<script>
  ...
  $(document).on('change','.d-list-province',function(){
    id = $(this).val();
    container = $(this).parent().parent().next().find('.d-province');
    $.ajax({
      headers: {
          'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
      },
      type:"post",
      url:"<?php echo e(route('ajax.province')); ?>",
      data:{'id':id},
      success:function(data){
        console.log(data);
        $('.d-a').show();
         $('.d-ready').show();
         $('.d-monny').find('input').attr('readonly',false);
