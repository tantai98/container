...
<form ui-jp="parsley" method="post" action="<?php echo e(route('order.submit.add')); ?>" enctype="multipart/form-data">
  ...
  <script>
      ...
      value = $(this).val();
        if($.trim(value)){
            $.ajax({
              ...
              type:"post",
              url:"<?php echo e(route('order.search.add')); ?>",
              data:{'key':value},
    ...
    $(document).on('click','.choose-product',function(){
      ...
      $.ajax({
            ...
            type:"post",
            url:"<?php echo e(route('order.checkbox.add')); ?>",
            data:{'id':id},
            success:function(data){
              console.log(data);
              id = data.product.id;
