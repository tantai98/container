...
<form ui-jp="parsley" method="post" action="<?php echo e(route('update.post')); ?>" enctype="multipart/form-data" id="form-edit-post">
...
<div class="navbar">
    	...
    	<?php if(session('admin')->can('luu_bai_viet')): ?>
		...
		<?php endif; ?>
		<?php if(session('admin')->can('dang_bai_viet')): ?>
							...
							<div class="row">
									<div class="col-sm-12">
							            ...
							            <div class="form-group">
							              <label>Tiêu đề</label>
							              <input type="text" class="form-control" name="post_title" value="<?php echo old('post_title'), isset($post) ? $post->title : ''; ?>" >                        
							            </div>
							            ...
							            <div class="form-group">
							               <div>   
							                ...
							                <p style="margin-bottom:10px;line-height:0px">
									               <img id="img_preview" style="max-height:200px;"  <?php if($post->img!=''): ?> src="<?php echo e(url($post->img)); ?>" <?php endif; ?>>
									              </p>
							            ...
							            <div class="form-group"> 
							              ...
							              <textarea name="post_des" class="form-control" rows="5"><?php echo old('post_des'), isset($post) ? $post->description : ''; ?></textarea>                       
							            </div>
										 <div class="form-group">
							                <?php
										...
										<div class="form-group">
									       ...
									       <input type="text" id="list_tag"  name="seo_tags"  class="form-control" autocomplete="off" value="<?php echo old('seo_tags'), isset($post_tags) ? $post_tags : ''; ?>">
								        </div>	
										      
							            <div class="form-group">
										   <label>SEO tiêu đề</label>
									       <input type="text" class="form-control" name="seo_title" value="<?php echo old('post_des'), isset($post) ? $post->SEO_title : ''; ?>">
										</div>	
										<div class="form-group">
										   <label>SEO Mô tả</label>
										   <textarea name="seo_des" class="form-control" rows="4"><?php echo old('seo_des'), isset($post) ? $post->SEO_des : ''; ?></textarea>
										</div>
										<div class="form-group">
										   <label>Struct Data</label>
										   <textarea name="seo_struct" class="form-control" rows="4"><?php echo old('seo_struct'), isset($post) ? $post->struct_data : ''; ?></textarea>
										</div>
										    ...
										    <div class="tab-pane p-v-sm <?php if($key==0): ?> active <?php endif; ?>" id="tab_<?php echo e($key+1); ?>">
										     <input type="hidden" name="content[id][]" value="<?php echo e($v3->id); ?>">
										      <div class="box m-t p-a-sm">
									              ...
									              <input type="text" class="form-control i_name" data-tab="1" name="content[name][]" value="<?php echo e($v3->name); ?>">
									            </div>
									            <div class="form-group">
									            ...
									            <?php if($key !=0): ?>
									          <!--   <div class="form-group"><button class="btn btn-warning btn-small close_tab" tab_id="<?php echo e($key+1); ?>" >Đóng tab</button>
									            </div> -->
									            <?php endif; ?>
									          </div>
										    </div>
...
<?php $__env->startSection('js-footer'); ?>
  ...
  <script src="<?php echo e(asset('summernote/dist/summernote-image-title.js')); ?>"></script>
	...
	$("#file_img_preview").change(function(){
	 ...
	 $(document).ready(function() {
	 	$("body").on('keyup', ".bootstrap-tagsinput input", function(evt) {
              ...
              $.ajax({
              	'type': 'post',
              	 'url':  '<?php echo e(route('tag.post.search')); ?>',
	 ...
	 $(function() {
		...
		<?php foreach($images as $img): ?>
			  ...
			  var myFile = {
				    name: "<?php echo e($img->img); ?>",
				    size: "<?php echo e(filesize(public_path().''.$img->img)); ?>"
			  };
